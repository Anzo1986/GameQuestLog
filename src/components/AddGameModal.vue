    ...
    <div class="relative bg-gray-900 w-full max-w-lg rounded-2xl shadow-2xl border border-gray-700 flex flex-col max-h-[85vh] overflow-hidden animate-in fade-in zoom-in duration-200">
      ...
      <div class="p-4 border-b border-gray-700 flex items-center gap-3">
        <template v-if="!showManualForm">
            ...
            />
             <!-- Manual Toggle -->
            <button @click="toggleManualMode" class="p-2 text-gray-400 hover:text-blue-400 hover:bg-gray-800 rounded-full transition-colors" title="Manually Add Game">
                <PenTool class="w-5 h-5" />
            </button>
        </template>
        <template v-else>
            <div class="flex items-center gap-2 w-full">
                <button @click="toggleManualMode" class="p-1 hover:bg-gray-800 rounded-full text-gray-400 hover:text-white transition-colors">
      ...
      <!-- MANUAL FORM -->
      <div v-if="showManualForm" class="p-6 space-y-4 overflow-y-auto">
          <div class="space-y-2">
          ...
          <div class="space-y-2">
              ...
              <div v-if="manualForm.image" class="mt-2 h-32 w-full rounded-lg overflow-hidden border border-gray-700 bg-gray-800 relative group">
                  <img :src="manualForm.image" class="w-full h-full object-cover" @error="$event.target.style.display='none'" />
                   <div class="absolute inset-0 flex items-center justify-center text-gray-500 text-xs" v-if="false">Preview</div> <!-- Fallback trick handled by img error mainly, or just keep simple -->
              </div>
          </div>
          ...
          <div class="pt-4 flex justify-end gap-3">
              <button @click="toggleManualMode" class="px-4 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition-colors">
                  Cancel
              </button>
              <button 
                @click="submitManualGame" 
                :disabled="!manualForm.name"
                class="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-2 rounded-lg shadow-lg font-medium transition-transform active:scale-95 flex items-center gap-2"
              >
